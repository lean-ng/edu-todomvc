...
export class StateService {
  ...
  async toggleTodoState(todo: Todo) {
    const updatedTodo = await this.store.update(todo.id, {
      completed: !todo.completed,
    });
    const ix = this.todos.findIndex((t) => t.id === todo.id);
    this.todos = [
      ...this.todos.slice(0, ix),
      updatedTodo,
      ...this.todos.slice(ix + 1),
    ];
  }
  ...
  async removeCompletedTodos() {
    await Promise.all(
      this.todos.filter((t) => t.completed).map((t) => this.store.remove(t.id))
    );
